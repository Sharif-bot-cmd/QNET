...
def search(req: Request, q: str = ""):
    ...
    Searches local databases first, then falls back to DuckDuckGo's open API.
    Displays results in QNET style — no external redirection, privacy-safe.
    """
    import html, re, urllib.parse, requests
    ...
    if not q:
        ...
        # --- 2. If local results exist, display them ---
        if matches:
            for item in matches:
        ...
        # --- 3. If no local results, try DuckDuckGo API ---
        else:
            ...
            try:
                ...
                if data.get("AbstractText"):
                    ...
                    results_html += f"""
                    <div class='search-result'>
                        <a class='result-title' href='/go?url={link}'>{title}</a>
                        <div class='result-snippet'>{desc}</div>
                ...
                for topic in related[:5]:
                    if isinstance(topic, dict) and "Text" in topic and "FirstURL" in topic:
                        ...
                        results_html += f"""
                        <div class='search-result'>
                            <a class='result-title' href='/go?url={u}'>{t}</a>
                            <div class='result-meta'>DuckDuckGo Related</div>
                        </div>
                        """

                if not results_html:
                    results_html = f"<p>No local or DuckDuckGo results found for <b>{safe_q}</b>.</p>"

            except Exception as e:
                print(f"[!] DuckDuckGo fetch error: {e}")
                results_html = f"""
                <p>Could not reach DuckDuckGo for <b>{safe_q}</b>.</p>
                <p><a href='/preview?url=https://duckduckgo.com/?q={safe_q}'>Open in preview</a></p>
                """

    # --- 4. Render full search page ---
    body = f"""
