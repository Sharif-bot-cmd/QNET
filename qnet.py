...
def search(req: Request, q: str = ""):
    """
    Local + external-style search.
    Displays DuckDuckGo-style previews for QNET content and safe external searches,
    all inside the QNET environment.
    """
    ...
    if not q:
        ...
        # --- Search inside QNET local databases ---
        ...
        # --- If no internal results, detect if query looks like a URL or IPFS hash ---
        if not matches:
            safe_q = html.escape(q)
            if re.match(r"^(http|https|ipfs)://", q):
                # Direct URL — show preview inside QNET
                preview_url = urllib.parse.quote(q, safe="")
                results_html = f"""
                <div class='search-result'>
                    <div class='result-title'>External Link</div>
                    <div class='result-snippet'>Detected URL — view safely inside QNET.</div>
                    <div class='result-meta'><a href='/preview?url={preview_url}'>Open Preview</a></div>
                </div>
                """
            elif re.match(r"^(Qm|bafy)[A-Za-z0-9]+$", q):
                # IPFS hash detected
                ipfs_url = f"ipfs://{q}"
                preview_url = urllib.parse.quote(ipfs_url, safe="")
                results_html = f"""
                <div class='search-result'>
                    <div class='result-title'>IPFS Content</div>
                    <div class='result-snippet'>Detected IPFS CID. Open preview inside QNET.</div>
                    <div class='result-meta'><a href='/preview?url={preview_url}'>View IPFS File</a></div>
                </div>
                """
            else:
                # Default external search fallback (DuckDuckGo sandbox)
                duckduckgo_url = f"https://duckduckgo.com/html/?q={urllib.parse.quote(q)}"
                results_html = f"""
                <div class='search-result'>
                    <div class='result-title'>No local results found for <b>{safe_q}</b></div>
                    <div class='result-snippet'>You can search the web safely through QNET’s sandbox preview.</div>
                    <div class='result-meta'><a href='/preview?url={duckduckgo_url}'>Search externally via DuckDuckGo</a></div>
                </div>
                """
        else:
            # --- Render internal QNET results ---
            for item in matches:
...
def preview(req: Request, url: str):
    """
    Loads a remote page preview (title, description, and favicon) safely inside QNET.
    Never opens new tabs or leaves the QNET environment.
    """
    ...
    if not (safe_url.startswith("http://") or safe_url.startswith("https://") or safe_url.startswith("ipfs://")):
        body = f"""
        <h2>⚠️ Unsafe or unsupported URL blocked</h2>
        <p>Requested: <code>{safe_url}</code></p>
        <p><a href='/' style='color:#0f0;'>Return to QNET</a></p>
        """
        return base_html("Blocked Preview", body, req=req, user=get_user(req))
    ...
    try:
        ...
        r = requests.get(
            ...
            timeout=5,
            headers={
                "User-Agent": "QNET/1.1 (Decentralized Browser)",
        ...
        if t:
            title = html.escape(t.group(1).strip())
        ...
        if d:
            desc = html.escape(d.group(1).strip())
        ...
        if f:
            fav_path = f.group(1)
            favicon = urllib.parse.urljoin(url, fav_path)

    except Exception as e:
        print(f"[!] Preview error for {url}: {e}")

    # --- Build preview card ---
    favicon_html = f"<img src='{favicon}' width='24' height='24' style='vertical-align:middle;margin-right:8px;border-radius:4px;'>" if favicon else ""
    body = f"""
    <div class='search-result'>
        <div class='result-title'>
            {favicon_html}<a href='/go?url={safe_url}'>{safe_url}</a>
        </div>
        <div class='result-snippet'><b>{title}</b></div>
        <div class='result-meta'>{desc}</div>
    </div>
    <p><a href='/' style='color:#0f0;'>Return to QNET</a></p>
    """

    return base_html("Preview", body, req=req, user=get_user(req))
