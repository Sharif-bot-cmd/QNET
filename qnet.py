...
def base_html(title, body, req: Request = None, user: str | None = None):
    ...
    and a unified look across all QNET pages.
    """
...
<style>
body {{
  background:#000;color:#0f0;font-family:monospace;margin:0;
}}
header {{
  border-bottom:1px solid #0f0;padding:10px;display:flex;flex-wrap:wrap;
  justify-content:space-between;align-items:center;
}}
nav {{
  display:flex;flex-wrap:wrap;gap:8px;justify-content:center;width:100%;margin-top:6px;
}}
nav a {{
  color:#0f0;text-decoration:none;padding:6px 10px;border-radius:6px;border:1px solid #0f0;
}}
nav a:hover {{
...
input,button,textarea {{
  background:#111;color:#0f0;border:1px solid #0f0;border-radius:6px;padding:8px;width:100%;margin:6px 0;
}}
button {{
  width:auto;cursor:pointer;background:#0f0;color:#000;
}}
.delete-btn {{
  background:#900;color:#fff;border:1px solid #f00;border-radius:4px;padding:4px 8px;cursor:pointer;
}}
iframe,video {{
  width:100%;height:360px;border:1px solid #0f0;border-radius:8px;
}}
...
#shield {{
  position:fixed;bottom:15px;right:15px;padding:10px 14px;border-radius:12px;
  background:{info['color']};color:#000;font-weight:bold;cursor:default;
...
<div id='shield' title='{html.escape(info['desc'])}'>🛡️ {html.escape(info['name'])}</div>
    ...
    # ✅ FIX: Preserve the session cookie so "Logged in as ..." stays visible across all pages
