...
# === UI Layer: Dynamic Node Status Indicator ===
def ui_status_badge(req: Request = None):
    ...
    mode, info = detect_mode()
    level = get_security_level()
    color = SECURITY_INFO[level]["color"]
...
def base_html(title, body, req: Request = None, user: str | None = None):
    ...
    try:
        ...
        print("[!] UI badge injection failed:", e)
    return html_resp
...
async def on_startup():
    """Kick off background maintenance and detection threads once FastAPI starts."""
    try:
        # network probe
        ...
        print(f"[!] Startup background thread error: {e}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        if host_ip.startswith("127.") or host_ip == "0.0.0.0":
            try:
                host_ip = requests.get("https://api.ipify.org", timeout=2).text.strip()
            except Exception:
                host_ip = "127.0.0.1"
    except Exception:
        host_ip = "127.0.0.1"
    ...
    # === 5️⃣ Initialize decentralized identity (DID) ===
    try:
    ...
    # === 6️⃣ Start peer discovery and listening ===
    try:
    ...
    # === 7️⃣ Start background sync + self-repair ===
    try:
        threading.Thread(target=sync_with_peers, daemon=True).start()
        start_self_repair_cycle()
    except Exception as e:
        print("[!] Background sync/self-repair failed:", e)

    # === 8️⃣ Launch FastAPI / QNET web service ===
